...
from Deasy_Labs import BaseModel, DeasyLabs, AsyncDeasyLabs
from Deasy_Labs._response import (
    APIResponse,
    BaseAPIResponse,
    AsyncAPIResponse,
    BinaryAPIResponse,
...
from Deasy_Labs._streaming import Stream
from Deasy_Labs._base_client import FinalRequestOptions
...
def test_extract_response_type_direct_class_missing_type_arg() -> None:
    with pytest.raises(
        RuntimeError,
        match="Expected type <class 'Deasy_Labs._response.AsyncAPIResponse'> to have a type argument at index 0 but it did not",
    ):
...
def test_response_parse_mismatched_basemodel(client: DeasyLabs) -> None:
    ...
    with pytest.raises(
        TypeError,
        match="Pydantic models must subclass our base model type, e.g. `from Deasy_Labs import BaseModel`",
    ):
